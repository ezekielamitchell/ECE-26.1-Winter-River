// ============================================================
// lighting_a.cpp — Lighting Circuit, Side A
// Chain: lv_dist_a → [lighting_a]
// Node ID: lighting_a | 277V Lighting (phase-to-neutral)
...
void setup() {
    ...
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(0, 0);
    display.println("lighting_a starting...");
    display.display();
...
void loop() {
    ...
    if (!mqtt.connected()) {
        String lwt_topic = "winter-river/lighting_a/status";
        String lwt_msg   = "{\"node\":\"lighting_a\",\"status\":\"OFFLINE\"}";

        if (!mqtt.connect("lighting_a", NULL, NULL,
        ...
        String online_msg = "{\"ts\":\"" + getTimestamp() +
                            "\",\"node\":\"lighting_a\",\"status\":\"ONLINE\"}";
        mqtt.publish("winter-river/lighting_a/status", online_msg.c_str(), true);

        // Subscribe to control topic
        mqtt.subscribe("winter-river/lighting_a/control");
        Serial.println("MQTT connected, subscribed to winter-river/lighting_a/control");
    }
    ...
    mqtt.publish("winter-river/lighting_a/status", payload.c_str());
