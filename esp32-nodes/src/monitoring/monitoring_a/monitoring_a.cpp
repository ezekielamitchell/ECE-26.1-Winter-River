// ============================================================
// monitoring_a.cpp — Monitoring / BMS Equipment, Side A
// Chain: lv_dist_a → [monitoring_a]
// Node ID: monitoring_a | 120V Monitoring & BMS
...
void setup() {
    ...
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(0, 0);
    display.println("monitoring_a starting...");
    display.display();
...
void loop() {
    ...
    if (!mqtt.connected()) {
        String lwt_topic = "winter-river/monitoring_a/status";
        String lwt_msg   = "{\"node\":\"monitoring_a\",\"status\":\"OFFLINE\"}";

        if (!mqtt.connect("monitoring_a", NULL, NULL,
        ...
        String online_msg = "{\"ts\":\"" + getTimestamp() +
                            "\",\"node\":\"monitoring_a\",\"status\":\"ONLINE\"}";
        mqtt.publish("winter-river/monitoring_a/status", online_msg.c_str(), true);

        // Subscribe to control topic
        mqtt.subscribe("winter-river/monitoring_a/control");
        Serial.println("MQTT connected, subscribed to winter-river/monitoring_a/control");
    }
    ...
    mqtt.publish("winter-river/monitoring_a/status", payload.c_str());
