// ============================================================
// cooling_a.cpp — Cooling System (CRAC/CRAH), Side A
// Chain: lv_dist_a → [cooling_a]
// Node ID: cooling_a | 480V Cooling
...
void setup() {
    ...
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(0, 0);
    display.println("cooling_a starting...");
    display.display();
...
void loop() {
    ...
    if (!mqtt.connected()) {
        String lwt_topic = "winter-river/cooling_a/status";
        String lwt_msg   = "{\"node\":\"cooling_a\",\"status\":\"OFFLINE\"}";

        if (!mqtt.connect("cooling_a", NULL, NULL,
        ...
        String online_msg = "{\"ts\":\"" + getTimestamp() +
                            "\",\"node\":\"cooling_a\",\"status\":\"ONLINE\"}";
        mqtt.publish("winter-river/cooling_a/status", online_msg.c_str(), true);

        // Subscribe to control topic
        mqtt.subscribe("winter-river/cooling_a/control");
        Serial.println("MQTT connected, subscribed to winter-river/cooling_a/control");
    }
    ...
    mqtt.publish("winter-river/cooling_a/status", payload.c_str());
