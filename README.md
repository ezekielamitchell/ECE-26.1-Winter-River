...
| Component Types        | 24 modules (12 Side A + 12 Side B)            | Utility, HV/MV transformer, MV switchgear, MV/LV transformer, LV switchgear, generator, ATS, UPS, cooling, 3× server_rack per side. |
...
│       ├── lv_switchgear/             # ⑤  480 V LV-bus disconnect (downstream of MV/LV xfmr)
...
│       └── server_rack/               # ⑩-⑫ Three 48 V DC racks per side (single shared source + build_flags)
...
| ⑤    | `lv_switchgear_a`        | 480 V LV-bus breaker (downstream of MV/LV xfmr)  | 480 V                      |
| ⑥    | `generator_a`            | Backup diesel generator (ATS secondary input)    | 480 V                      |
| ⑦    | `ats_a`                  | LV transfer switch — LV switchgear or generator  | 480 V                      |
| ⑧    | `ups_a`                  | Uninterruptible power supply (feeds 3 racks)     | 480 V AC                   |
| ⑨    | `cooling_a`              | CRAC/CRAH fan bank (55 fans on this side)        | 480 V AC                   |
| ⑩-⑫ | `server_rack_a{1..3}`    | 48 V DC IT racks (single-fed from `ups_a`)       | 48 V DC                    |
...
| `scripts/init_db.sql`                 | PostgreSQL schema (24-node seed data, `secondary_parent_id` for ATS) |
